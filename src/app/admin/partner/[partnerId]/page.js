...
const Page = ({ params }) => {
  ...
  return (
    <Box>
      <Container>
        <Box>
          <Typography sx={{textAlign: "center", fontSize: "20px", fontWeight: "700", marginY: "5px"}}>Partner Detail</Typography>
        </Box>
        {partner && (
            ...
            {partner.isVerified ? (
              <Box sx={{width: "100%", display: "flex", flexWrap: "wrap",gap: "10px"}}>
                <Box sx={{display: "flex", gap: "15px"}}>
                  <Typography sx={{fontWeight: "bold"}}>Verification Status : </Typography>
                  <Typography>Verified</Typography>
                </Box>
                <Box sx={{display: "flex", gap: "15px"}}>
                  <Typography sx={{fontWeight: "bold"}}>Cases Assigned : </Typography>
                ...
                <Box sx={{display: "flex", gap: "15px"}}>
                  ...
                  <Typography>{dayjs(partner.joiningDate).format("DD-MM-YYYY")} </Typography>
                </Box>
              </Box>
            ) : (
              <Box sx={{ display: "flex", gap: "10px", width: "100%", alignItems: "center" }}>
                ...
                <Button onClick={verifyPartner} disabled={isVerifying}>{isVerifying? "Please wait": "verify"}</Button>
              </Box>
            )}
          </Box>
        )}
